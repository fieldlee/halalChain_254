...
function restartNetwork() {
	...
	cd network
	docker-compose -f docker-compose-couch.yaml  -f docker-compose-e2e.yaml  down
	dkcl
	dkrm
	...
	#Start the network
	docker-compose -f docker-compose-couch.yaml  -f docker-compose-e2e.yaml  up -d
	cd -
	echo
}
...
function replaceClientPrivKey(){
  ...
  # replace ca key
  cp network/docker-compose-e2e-template.yaml network/docker-compose-e2e.yaml
  cd network/crypto-config/peerOrganizations/ageli.com/ca
  PRIV_CA_KEY=$(ls *_sk)
  echo "$PRIV_CA_KEY"
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_CA_KEY}/g" network/docker-compose-e2e.yaml


  cd network/crypto-config/peerOrganizations/creator.com/ca
  PRIV_CA_KEY=$(ls *_sk)
  echo "$PRIV_CA_KEY"
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA2_PRIVATE_KEY/${PRIV_CA_KEY}/g" network/docker-compose-e2e.yaml


  cd network/crypto-config/peerOrganizations/transfer.com/ca
  PRIV_CA_KEY=$(ls *_sk)
  echo "$PRIV_CA_KEY"
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA3_PRIVATE_KEY/${PRIV_CA_KEY}/g" network/docker-compose-e2e.yaml

  cd network/crypto-config/peerOrganizations/seller.com/ca
  PRIV_CA_KEY=$(ls *_sk)
  echo "$PRIV_CA_KEY"
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA4_PRIVATE_KEY/${PRIV_CA_KEY}/g" network/docker-compose-e2e.yaml
